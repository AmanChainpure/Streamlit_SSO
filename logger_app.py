...
def theme_for_page(username):
    ...
    try:
        ...
        logger.error(f"Error while creating theme for page for user {username}: {e}")
    ...
    try:
        ...
        for uploaded_file in uploaded_files:
            ...
            if text_token_count > 50000:
                ...
                logger.info("PDF data saved to Azure blob.")
                extracted_text += text

        return extracted_text

    except Exception as e:
        logger.error(f"Error while opening the file: {e}")
...
def confidentiality_agreement():
    st.write("""
    By accessing this AI assistance, you agree to the following terms: 
    ...
    This agreement supplements any existing confidentiality agreement you have with the company
    """)
...
def main():
    ...
    with st.sidebar:
        ...
        if user:
            ...
            if "agreement" not in st.session_state:
                st.write("Before you get started, Please read and agree to the confidentiality agreement before proceeding.")
                if st.button("View"):
    ...
    try:
        ...
        logger.error(f"Error in processing document upload: {e}")
    ...
    try:
        ...
        logger.error(f"Error in loading new chat session: {e}")
        st.sidebar.write("Error in loading new chat session.")

    with st.sidebar:
        history_button_first = st.button("Chat History", type='primary', use_container_width=True)
        if history_button_first:
            ...
            try:
                ...
                for session in chat_history:
                    ...
                    for conversation in session["conversations"]:
                        ...
                        key = "Chatbot"
                        st.session_state.chatbotanswer.append({key: conversation.get('Chatbot')})
                        st.sidebar.write(f"**Assistant:** {conversation.get('Chatbot')}")
            except Exception as e:
                logger.warning(f"No chat history available for user {username}. Error: {e}")
                st.sidebar.write(f"Hi {username}, you don't have any chat history. Please start a conversation first.")
